...
@Schema(description = "管理后台 - CRM对公客户扩展表(对公客户特有基本信息)分页 Request VO")
@Data
public class CompanyCustomerPageReqVO extends PageParam {

    @Schema(description = "客户ID(外键,关联crm_customer.id,唯一)", example = "26736")
    ...
    @Schema(description = "是否上市公司(0=否, 1=是)")
    private Boolean isListed;

    @Schema(description = "是否小微企业(0=否, 1=是)")
    private Boolean isSmallEnterprise;

    @Schema(description = "是否集团客户(0=否, 1=是)")
    private Boolean isGroupCustomer;

    @Schema(description = "是否有进出口权(0=否, 1=是)")
    private Boolean isImportExport;

    @Schema(description = "是否我行关联方(0=否, 1=是)")
    private Boolean isRelatedParty;

    @Schema(description = "是否网银签约客户(0=否, 1=是)")
    private Boolean isEbankSigned;

    @Schema(description = "是否涉农企业(0=否, 1=是)")
