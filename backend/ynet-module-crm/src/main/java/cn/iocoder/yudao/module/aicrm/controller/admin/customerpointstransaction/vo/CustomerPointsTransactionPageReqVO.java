package cn.iocoder.yudao.module.aicrm.controller.admin.customerpointstransaction.vo;
...
@Schema(description = "管理后台 - 客户积分消费明细表（积分交易流水表）分页 Request VO")
@Data
public class CustomerPointsTransactionPageReqVO extends PageParam {

    @Schema(description = "客户ID（关联 crm_customer 主表）", example = "16467")
    private Long customerId;

    @Schema(description = "积分账户ID（关联 crm_customer_points 表）", example = "1188")
    ...
    @Schema(description = "兑换渠道（字典: aicrm_exchange_channel，crm=CRM系统，website=网银，mobile_banking=手机银行，wechat=微信银行，counter=柜台）")
    ...
    @Schema(description = "订单消耗积分")
    private Long orderConsumedPoints;

    @Schema(description = "订单状态（字典: aicrm_order_status，10=待处理，20=处理中，30=已完成，40=已取消，50=已退款）", example = "2")
    private String orderStatus;

    @Schema(description = "订单审批状态（字典: aicrm_order_approval_status，1=待审批，2=审批通过，3=审批拒绝）", example = "2")
    ...
    @Schema(description = "交易类型（字典: aicrm_points_transaction_type，earn_deposit=存款获取，earn_wealth=理财获取，earn_transaction=交易获取，earn_activity=活动获取，earn_gift=赠送获取，consume_redeem=兑换消费，consume_deduct=扣减，expire=过期失效，refund=退款，adjust=调整）", example = "2")
    ...
    @Schema(description = "交易状态（字典: aicrm_transaction_status，pending=待处理，processing=处理中，success=成功，failed=失败，cancelled=已取消）", example = "1")
