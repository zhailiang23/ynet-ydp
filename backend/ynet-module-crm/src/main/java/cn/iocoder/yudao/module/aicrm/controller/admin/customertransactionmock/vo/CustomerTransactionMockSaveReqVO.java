package cn.iocoder.yudao.module.aicrm.controller.admin.customertransactionmock.vo;
...
@Schema(description = "管理后台 - 客户交易明细信息表（Mock数据）新增/修改 Request VO")
@Data
public class CustomerTransactionMockSaveReqVO {
    ...
    @NotEmpty(message = "交易状态（字典: aicrm_transaction_status，success=成功，failed=失败，pending=处理中）不能为空")
    private String transactionStatus;

    @Schema(description = "交易方向（字典: aicrm_transaction_direction，in=转入，out=转出）", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotEmpty(message = "交易方向（字典: aicrm_transaction_direction，in=转入，out=转出）不能为空")
