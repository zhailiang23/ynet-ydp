...
@Schema(description = "管理后台 - 客户渠道行为信息 Response VO")
@Data
@ExcelIgnoreUnannotated
public class CustomerChannelBehaviorRespVO {
    ...
    @ExcelProperty("操作行为（字典: aicrm_operation_action，click=点击，login=登录，browse=浏览，query=查询，submit=提交，download=下载，upload=上传，search=搜索）")
    ...
    @ExcelProperty("操作对象（字典: aicrm_operation_object，page=页面，button=按钮，input=输入框，link=链接，menu=菜单，form=表单）")
    private String operationObject;

    @Schema(description = "当前页面标识（如：积分、活动详情页、登录、贷款产品页、我的积分等）", requiredMode = Schema.RequiredMode.REQUIRED)
    @ExcelProperty("当前页面标识（如：积分、活动详情页、登录、贷款产品页、我的积分等）")
    private String currentPageIdentifier;

    @Schema(description = "当前页面名称（页面code，如：index、loan、my_page、act_details）")
    @ExcelProperty("当前页面名称（页面code，如：index、loan、my_page、act_details）")
    ...
    @ExcelProperty("当前页面名称（中文名称，如：首页、我的贷款、我的、我的积分、饭票新人活动详情页）")
    private String currentPageName;

    @Schema(description = "页面停留时间(S)", requiredMode = Schema.RequiredMode.REQUIRED)
    @ExcelProperty("页面停留时间(S)")
    ...
    @ExcelProperty("渠道类型（字典: aicrm_channel_type，mobile_banking=手机银行，web_banking=网上银行，wechat=微信银行，crm=CRM系统，counter=柜台，atm=ATM，call_center=电话银行）")
    private String channelType;

    @Schema(description = "设备类型（字典: aicrm_device_type，ios=iOS，android=Android，web=网页，h5=H5，pc=PC）", example = "2")
    @ExcelProperty("设备类型（字典: aicrm_device_type，ios=iOS，android=Android，web=网页，h5=H5，pc=PC）")
    ...
    @ExcelProperty("网络类型（字典: aicrm_network_type，wifi=WiFi，4g=4G，5g=5G，ethernet=有线网络）")
    ...
    @ExcelProperty("页面参数（JSON格式）")
    private String pageParams;

    @Schema(description = "操作结果（字典: aicrm_operation_result，success=成功，failed=失败，cancelled=取消）")
    @ExcelProperty("操作结果（字典: aicrm_operation_result，success=成功，failed=失败，cancelled=取消）")
    ...
    @ExcelProperty("业务类型（字典: aicrm_behavior_business_type，account=账户，loan=贷款，wealth=理财，transfer=转账，payment=支付，points=积分，activity=活动）")
    ...
    @ExcelProperty("是否转化（是否产生业务）")
    private Boolean isConversion;

    @Schema(description = "转化类型（字典: aicrm_conversion_type）", example = "2")
    @ExcelProperty("转化类型（字典: aicrm_conversion_type）")
