...
@Schema(description = "管理后台 - 客户卡券信息表（零售客户特有） Response VO")
@Data
@ExcelIgnoreUnannotated
public class CustomerCouponRespVO {
    ...
    @ExcelProperty("卡券类型（字典: aicrm_coupon_type，discount=折扣券，cash=代金券，gift=礼品券，rate=利率券，fee_waiver=手续费减免券）")
    ...
    @ExcelProperty("卡券状态（字典: aicrm_coupon_status，unused=未使用，used=已使用，expired=已过期，cancelled=已作废，locked=已锁定）")
    ...
    @ExcelProperty("发放来源（字典: aicrm_coupon_issue_source，system_gift=系统赠送，activity_reward=活动奖励，birthday_gift=生日礼物，point_exchange=积分兑换，manual_issue=手动发放）")
    ...
    @ExcelProperty("发放渠道（字典: aicrm_coupon_channel，mobile_banking=手机银行，web_banking=网上银行，wechat=微信银行，counter=柜台，crm=CRM系统）")
    private String issueChannel;

    @Schema(description = "关联活动ID（如果来源于营销活动）", example = "17381")
    @ExcelProperty("关联活动ID（如果来源于营销活动）")
    private Long issueActivityId;

    @Schema(description = "关联活动名称", example = "张三")
    @ExcelProperty("关联活动名称")
    ...
    @ExcelProperty("使用场景（字典: aicrm_coupon_use_scenario，online_purchase=线上购买，counter_purchase=柜台办理，app_purchase=APP办理，wealth_purchase=理财购买，loan_apply=贷款申请）")
    private String useScenario;

    @Schema(description = "使用渠道（字典: aicrm_coupon_channel）")
    @ExcelProperty("使用渠道（字典: aicrm_coupon_channel）")
    private String useChannel;

    @Schema(description = "使用产品代码")
    @ExcelProperty("使用产品代码")
    private String useProductCode;

    @Schema(description = "使用产品名称", example = "芋艿")
    @ExcelProperty("使用产品名称")
