-- ----------------------------
-- 网格信息表、客户归属网格表、网格归属调整历史表 (重新设计)
-- 设计原则: 客户归属网格表每条记录只包含一个网格信息
...
CREATE TABLE `crm_customer_grid_assignment` (
  ...
  `grid_code` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '网格编号',
  `grid_name` varchar(200) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '网格名称',
  `grid_type` varchar(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '网格类型（physical=物理网格，virtual=虚拟网格）',
  `grid_manager_user_id` bigint NULL DEFAULT NULL COMMENT '网格管理员用户ID（关联 system_users.id）',
...
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = '客户归属网格关系表（每条记录只包含一个网格信息）';
...
CREATE TABLE `crm_customer_grid_history` (
  ...
  INDEX `idx_tenant_id`(`tenant_id` ASC) USING BTREE COMMENT '租户ID索引'
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = '客户归属网格调整历史表（记录单个网格的变更历史）';
