...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    modalApi.lock();
    // 提交表单
    const data = (await formApi.getValues()) as GridHuinongMarketingApi.HuinongMarketing;
    try {
      await (formData.value?.id ? updateHuinongMarketing(data) : createHuinongMarketing(data));
      // 关闭并提示
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    modalApi.lock();
    try {
      formData.value = await getHuinongMarketing(data.id);
      // 设置到 values
