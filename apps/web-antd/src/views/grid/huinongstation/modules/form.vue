...
const getTitle = computed(() => {
  return formData.value?.id
    ? $t('ui.actionTitle.edit', ['惠农站点信息'])
    : $t('ui.actionTitle.create', ['惠农站点信息']);
});
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    modalApi.lock();
    // 提交表单
    const data = (await formApi.getValues()) as GridHuinongStationApi.HuinongStation;
    try {
      await (formData.value?.id ? updateHuinongStation(data) : createHuinongStation(data));
      // 关闭并提示
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    modalApi.lock();
    try {
      formData.value = await getHuinongStation(data.id);
      // 设置到 values
