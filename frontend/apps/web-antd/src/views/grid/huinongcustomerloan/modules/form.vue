...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    modalApi.lock();
    // 提交表单
    const data = (await formApi.getValues()) as GridHuinongCustomerLoanApi.HuinongCustomerLoan;
    try {
      await (formData.value?.id ? updateHuinongCustomerLoan(data) : createHuinongCustomerLoan(data));
      // 关闭并提示
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    modalApi.lock();
    try {
      formData.value = await getHuinongCustomerLoan(data.id);
      // 设置到 values
